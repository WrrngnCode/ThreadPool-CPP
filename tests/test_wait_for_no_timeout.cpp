...
int main() {
    ...
    std::cout << "Waiting for tasks to finish... Timeout in " << TIME_NEEDED_EST * 1.5 << " ms. " << endl;


    WaitStatus ret = pool.wait_for(TIME_NEEDED_EST * 1.5);

    assert(ret == WaitStatus::DONE_BEFORE_TIMEOUT);  
    assert(pool.getTasksPending() == 0);
    assert(pool.getTasksQueued() == 0);

    if (ret == WaitStatus::DONE_BEFORE_TIMEOUT) {
        std::cout << "Success: All tasks were finished before timeout." << endl;
    } else if (ret == WaitStatus::TIMEOUT) {
        std::cout << "Error: Timeout shouldn't have happened." << endl;
    }
